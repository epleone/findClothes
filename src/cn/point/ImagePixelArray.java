    ...
    public ImagePixelArray doubled() {
        ...
        for (int y = 0; y < this.height - 1; y++) {
            for (int x = 0; x < this.width - 1; x++) {
                ...
                db.data[2 * (x + y * nw) + nw] = (this.data[x + y * this.width] + this.data[x + (y + 1) * this.width]) / 2.0f; // 涓嬭竟鎻掑?

                db.data[2 * (x + y * nw) + 1] = (this.data[x + y * this.width] + this.data[x + y * this.width + 1]) / 2.0f; // 鍙宠竟鎻掑?
